...
const [showSchoolBlock, setShowSchoolBlock] = useState<ShowSchoolBlockTypes>({0: false});
    ...
    <>
      ...
      <div className="max-w-full min-h-[100vh] pb-[16rem] md:pb-[12rem] md:mr-[12.5rem] lg:mr-[15.6rem] md:ml-[4.4rem] relative">
        ...
        {filteredSchools.map((school, schoolIndex) => (
          <div
            key={schoolIndex}
            className={`shadow-md mx-2 py-2 ${
              schoolIndex === 0 ? "mt-[8rem] md:mt-[5rem]" : "mt-8"
            } rounded-md`}
          >
            <div className="flex flex-col items-center justify-between mx-2 md:ml-[2rem] gap-3 lg:flex-row lg:justify-between lg:items-top">
            ...
            <Image src={Popup} alt="Mostrar temas" width={20} height={20} className={`mx-auto my-3 cursor-pointer ${showSchoolBlock[schoolIndex] ? 'rotate-180' : 'rotate-0'}`} onClick={() => handleSchoolBlockClick(schoolIndex)} />
          </div>
        ))}

        <section className="overflow-x-scroll mt-7 mx-2 shadow-md rounded-md px-5">
